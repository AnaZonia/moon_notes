...
var Note = class {
  ...
  async share() {
    ...
    this.status = new StatusMessage("Sharing note...", 0 /* Default */, 30 * 1e3);
    ...
    try {
      ...
      this.css = [...Array.from(document.styleSheets)].map((x) => {
        try {
          return [...Array.from(x.cssRules)].map((x2) => x2.cssText).join("");
        } catch (e) {
          console.log(e);
          return "";
        }
    ...
    this.template.elements.push(getElementStyle("preview", this.domCopy.getElementsByClassName("markdown-preview-view markdown-rendered")[0]));
    this.template.elements.push(getElementStyle("pusher", this.domCopy.getElementsByClassName("markdown-preview-pusher")[0]));
  ...
  async uploadCss() {
    ...
    const cssNotice = new StatusMessage(cssNoticeText, 1 /* Info */, 12e4);
...
var API = class {
  ...
  async postRaw(endpoint, data, retries = 3) {
    const headers = {
      ...await this.authHeaders(),
