...
validate - I could fit it to some time in the past (2015) and then compare it with 2022 and see how it works.
could exclude the ones that flip back and forth because of uncertainty -  

check how much of the area would become savanna

what is the predicted overall biomass across the three scenarios (plus maps)

compare the predictions to a baseline of current mature forests

consider land cover changes under the three scenarios - see WHERE forests are regrowing. is all of that area going to be savanna?

make maps for 2100 with areas that are transitioning

see how the transitions will affect the biomass of secondary forests  
...
if I consider the current mature forest as asymptote

## 2026/08/28
Having issues downloading CMIP6 data from the import_CMIP6.py script, used for the IPCC report. (https://cds.climate.copernicus.eu/datasets/projections-cmip6?tab=overview). The API code generated by the Climate Data Store also doesn't work, so it is not my fault. I am moving on without the historical data pre-2010 (that for some reason I accidentally did not download) and using 2010-2015 as my historical data reference.

Finished organizing the raster files in CMIP6_raster to add the new CMIP6 predictors to google earth engine. In the future, need to try again on copernicus to make sure I am downloading a longer historical range and including the year 2015 (that somehow got skipped here) for the proper inclusion of the predictors in historical and future data.
...
**Tragedy of the commons** -> the idea that once a good becomes common, it is more prone to exploitation because short-term individual benefits outweigh long-term collective benefits. [Has been contested by Elinor Ostrom.](https://www.aei.org/articles/elinor-ostrom-and-the-solution-to-the-tragedy-of-the-commons/), the same person who came up with the [[Ostrom Framework]].
